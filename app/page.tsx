...
export default function Home(){
  ...
  const importRows=useMemo(()=>sortedRows.filter(r=>Number.isFinite(Number(r.amount))&&Number(r.amount)>0),[sortedRows]);
  ...
  function exportCsv(){
    ...
    const data=importRows.map(r=>[r.date.split("-").reverse().join("."),r.category,r.merchant,r.realised?`Дата проводки: ${r.realised.split("-").reverse().join(".")}; источник: mHB klik`:"Источник: mHB klik",account,r.kind==="expense"?r.amount:"","","",r.kind==="income"?r.amount:"","","",""]);
